...
#ifndef PPV_DISTRIBUTED_MARKER_QUEUE_DISC_H
#define PPV_DISTRIBUTED_MARKER_QUEUE_DISC_H
...
#include "ns3/ppv-globalRate-singleton.h"
 ...
 *
 */
class PpvDistributedMarkerQueueDisc : public PpvMarkerQueueDisc {
public:
  /**
  ...
  /**
   * \brief PpvDistributedMarkerQueueDisc constructor
   *
   * Creates a queue with a depth of 1000 packets by default
   */
  PpvDistributedMarkerQueueDisc ();

  virtual ~PpvDistributedMarkerQueueDisc();
...
private:
  ...
  virtual void InitializeParams (void);
	...
	void PPV_Print_Stat (float tnow, long tb_d_new, float R, float R_long, float Rcalc);
	void LogMarkerCTV (float tnow, float R, float ctv);
	void updateCTV ();
	void updateRate(int packetsize, float tb_time, float dt, long& tb_d, float& R_old, float& R);
	...
	PpvGlobalRateSingleton* globalRate;
    ...
    float R_long; // bytes/sec
    float stored_R_long; // bytes/sec
    float stored_R_long_sync_last_time; // sec
	float RlongSendingTimeSec;
	float aggrRateRecvTimeSec;
	float flowTimeoutSec;
    float R_old_long; // bytes/sec
    float tb_time_long; // token bucket size in sec
    long tb_d_long; // bytes in token bucket
